...
class MultiCheckComponent3 with ChangeNotifier, ComponentBase implements SerializableComponent {
  ...
  MultiCheckComponent3(id, groupId, componentLabel, {this.columns = 5, this.hasSelectAll = false, this.selectAll = false, this.columnWidth, this.saveState = true}){
    super.id = id;
  ...
  Widget selectAllCheckBox( ) {
    ...
    var checkIcon = IconButton(
      onPressed: () {
        print("Pressed select all $allSelected ");
        print("\t$selected");
        if( !allSelected){
          ...
          for( var opt in options ){
            // print("\t De-selecting $opt (${selected.contains(opt)})");
            if( selected.contains(opt)) {
              deselect(opt);
            }
          }
